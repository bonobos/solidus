namespace 'spree:migrations:migrate_user_addresses' do
  ...
  task up: :environment do
    Spree.user_class.find_each(batch_size: 500) do |user|
      ...
      past_addresses = user.orders.includes(:ship_address, :bill_address).flat_map { |o| [o.bill_address, o.ship_address] }.compact.uniq

      past_addresses.each do |address|
        user.save_in_address_book(address, false)
      end
      current_addresses.each do |address|
        user.save_in_address_book(address, true)
      end
