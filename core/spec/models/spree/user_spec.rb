  ...
  context "#last_incomplete_order" do
    ...
    context "persists order address" do
      ...
      it "associates both the bill and ship address to the user" do
        ...
        expect(user.user_addresses.find_by(address: order.bill_address)).to_not be_nil
        expect(user.user_addresses.find_by(address: order.ship_address)).to_not be_nil
      end
    end
